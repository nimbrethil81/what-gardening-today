-- ============================================================================
--  Account deletion test suite — SECOND HALF ONLY (diagnostic)
--
--  The companion to the FIRSTHALF file. The first half passed, so this runs
--  the remaining checks: handing a garden over, a member leaving, one of two
--  owners leaving, who gets promoted, a user with no garden, a user with two
--  gardens at once, and finally that no garden is left with nobody in it.
--
--  Same throwaway data, same rollback at the end: nothing is kept.
--
--  Both diagnostic files are superseded once 12_account_deletion_test.sql is
--  fixed; delete them then.
--
